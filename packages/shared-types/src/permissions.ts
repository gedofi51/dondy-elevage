/**
 * Catalogue des permissions — déplacé depuis
 * apps/api/src/common/rbac/permissions.constants.ts (Phase 9) pour être
 * partagé front/back sans duplication ; ce fichier de l'API n'est plus
 * qu'un ré-export. RBAC piloté par données (voir schema.prisma, modèles
 * Role/Permission/RolePermission). Cette liste n'est PAS un enum métier
 * figé : elle décrit seulement les codes que le code applicatif connaît
 * et vérifie via PermissionsGuard côté API — seule application réelle,
 * jamais côté front (affichage conditionnel uniquement).
 *
 * `platform.manage` est un cas particulier : réservé au rôle système
 * Super Admin, il autorise les opérations transverses à toutes les
 * fermes (ex. création d'une nouvelle ferme) — jamais accordé à un rôle
 * de ferme.
 */
