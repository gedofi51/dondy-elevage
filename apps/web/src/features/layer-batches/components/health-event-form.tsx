...
import type { CreateLayerHealthEventInput } from '@dondy-elevage/shared-types';
...
import { useCreateLayerHealthEvent } from '../hooks';
import {
  createLayerHealthEventSchema,
  type CreateLayerHealthEventFormInput,
  type CreateLayerHealthEventFormValues,
} from '../schemas';
import {
...
export function HealthEventForm({ batchId, onSuccess }: { batchId: string; onSuccess?: () => void }) {
  ...
  const createMutation = useCreateLayerHealthEvent(batchId);
  const {
    register,
    control,
    watch,
  ...
  } = useForm<CreateLayerHealthEventFormInput, unknown, CreateLayerHealthEventFormValues>({
    resolver: zodResolver(createLayerHealthEventSchema),
    defaultValues: { date: todayIsoDate(), type: 'TRAITEMENT', status: 'PREVU' },
  });
  const itemId = watch('itemId');
  const status = watch('status');

  async function onSubmit(values: CreateLayerHealthEventFormValues) {
    try {
      const input: CreateLayerHealthEventInput = {
        ...values,
