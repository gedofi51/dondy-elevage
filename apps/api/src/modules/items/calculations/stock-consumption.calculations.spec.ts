import { computeStockConsumptionInstructions } from './stock-consumption.calculations';

describe('computeStockConsumptionInstructions', () => {
  it('première saisie : SORTIE intégrale (scénario §16-F, 50 kg distribués)', () => {
    expect(computeStockConsumptionInstructions(null, 0, 'item-1', 50)).toEqual([
  ...
  it('même article, quantité augmentée : SORTIE du delta seulement', () => {
    expect(computeStockConsumptionInstructions('item-1', 50, 'item-1', 70)).toEqual([
  ...
  it('même article, quantité réduite : ENTREE (RETOUR) de la réduction', () => {
    expect(computeStockConsumptionInstructions('item-1', 70, 'item-1', 50)).toEqual([
      { itemId: 'item-1', type: 'ENTREE', reason: 'RETOUR', quantity: 20 },
    ]);
  });

  it('même article, quantité inchangée : aucune instruction', () => {
    expect(computeStockConsumptionInstructions('item-1', 50, 'item-1', 50)).toEqual([]);
  });

  it('article changé : RETOUR intégral de l’ancien + SORTIE intégrale du nouveau (jamais un delta croisé)', () => {
    expect(computeStockConsumptionInstructions('item-1', 50, 'item-2', 30)).toEqual([
  ...
  it('article retiré (feedItemId remis à null) : RETOUR intégral, rien de plus', () => {
    expect(computeStockConsumptionInstructions('item-1', 50, null, 0)).toEqual([
      { itemId: 'item-1', type: 'ENTREE', reason: 'RETOUR', quantity: 50 },
    ]);
  });

  it('aucun article jamais renseigné : aucune instruction', () => {
    expect(computeStockConsumptionInstructions(null, 0, null, 0)).toEqual([]);
  });
});
